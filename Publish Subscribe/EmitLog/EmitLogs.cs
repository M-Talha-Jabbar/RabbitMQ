            ...
            using (var connection = factory.CreateConnection())
            {
                using (var channel = connection.CreateModel())
                {
                    ...
                    channel.BasicPublish(exchange: "logs", // Publishing the message into 'logs' exchange.
                                         routingKey: "", // Since the Exchange Type is 'Fanout', it just broadcasts all the messages it receives to all the queues it knows. So no need to define routingKey value.
